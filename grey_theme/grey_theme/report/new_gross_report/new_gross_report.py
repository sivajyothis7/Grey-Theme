	...
	group_wise_columns = frappe._dict(
		{
			...
			"project": ["project", "base_amount", "buying_amount", "gross_profit", "gross_profit_percent", "markup_percent"],
			"cost_center": [
				"cost_center",
				"base_amount",
				"buying_amount",
...
def get_data_when_grouped_by_invoice(columns, gross_profit_data, filters, group_wise_columns, data):
	...
	for src in gross_profit_data.si_list:
		...
		group = group_wise_columns.get(scrub(filters.group_by)) or []
		...
		frappe._dict(
			{
				...
				"markup_percent": flt(
					(total_gross_profit / total_buying_amount) * 100.0,
					cint(frappe.db.get_default("currency_precision")) or 3,
				)
...
def get_data_when_not_grouped_by_invoice(gross_profit_data, filters, group_wise_columns, data):
	...
	group_columns = group_wise_columns.get(scrub(filters.group_by)) or []
	...
	markup_percent = (total_gross_profit / total_buying_amount * 100.0) if total_buying_amount else 0	
	...
	total_row = {
		...
		"markup_percent": flt( markup_percent, currency_precision),
	}
	...
	column_map = frappe._dict(
		{
			...
			"gross_profit_percent": {
				"label": _("Margin Percent"),
				"fieldname": "gross_profit_%",
				"fieldtype": "Percent",
				"width": 100,
			},
			"markup_percent": {
				"label": _("Markup Percent"),
	...
	def process(self):
		...
		for row in reversed(self.si_list):
			...
			if row.buying_amount:
				row.markup_percent = flt(
					(row.gross_profit / row.buying_amount) * 100.0,
					self.currency_precision,
				)
			else:
				row.markup_percent = 0.0
			# add to grouped
